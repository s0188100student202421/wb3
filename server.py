from mysql.connector import Errorimport cgi
class HttpProcessor(BaseHTTPRequestHandler):
    def do_GET(self):        self.send_response(200)
        self.send_header('content-type', 'text/html')        self.end_header()
        with open('form.html', 'r') as file:
            html_content = file.read()            self.wfile.write(html_content)
    def do_POST(self):        
        ...
        try:
            ...
            if connection.is_connected():                
                ...
                cursor.execute(""" INSERT INTO aplications_languages(fio, phone, email, date, gender, bio, check_agreement)
                    VALUES (%s, %s, %s, %s, %s, %s, %s)  """, (fio, phone, email, date, gender, bio, bool(check)))
                user_id = cursor.lastrowid
                for lang in languages:                    
                    cursor.execute("""
                        INSERT INTO programming_languages(user_id, lang) VALUES (%s, %s)
                    """, (user_id, lang))                
                connection.commit()
                cursor.close()                
                connection.close()
